...
export function setupCSSForm() {
  ...
  document.getElementById('editor').addEventListener('click', function (e) {
    ...
    if (rules) {
      var form = document.getElementById('css-form');

      form.querySelectorAll('.dynamic-field').forEach((field) => {
        field.remove();
      });
      ...
      if (selectedComponent.id !== 'editor') {
        rules.fields.forEach((field) => {
          ...
          var submitButton = form.querySelector('#submit-css');
        ...
        document.getElementById('remove-button').setAttribute('disabled', '');
        populateForm(selectedComponent);
      }
    }
  });
